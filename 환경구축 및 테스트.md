...
sudo tc qdisc del dev lo root
```
2) 딜레이 주입 (ex. 200ms: RTT 왕복 지연 400ms 환경 주입)
```
sudo tc qdisc del dev lo root
...
sudo tc qdisc add dev lo root netem loss 20%
```
4) 딜레이+손실 복합 주입 (ex. 200ms + 20%)
```
sudo tc qdisc del dev lo root
sudo tc qdisc add dev lo root netem delay 200ms loss 20%
```

#### 수집 종료 후 네트워크 정상화 및 결과 확인
1) 네트워크 원상 복구
```
sudo tc qdisc dev lo root 2>/dev/null
(sudo tc qdisc del dev lo root)
```
2) 각 csv 파일에 데이터가 몇 줄씩 모였는지 확인: 각 데이터 개수 및 합계 출력
```
wc -l data/*.csv
```
...
1단계: 시나리오별 데이터 수집
- 정상(Normal): 설정 없이 client.py 실행

- 지연(Delay): sudo tc qdisc add dev lo root netem delay 500ms 적용 후 실행 

- 손실(Loss): 설정을 지우고(del), sudo tc qdisc add dev lo root netem loss 20% 적용 후 실행

- 혼합(Mixed): 설정을 지우고, sudo tc qdisc add dev lo root netem delay 500ms loss 20% 적용 후 실행



2단계: 데이터셋 전처리 (Labeling)   
머신러닝 모델이 공부하려면 "이 줄은 정상이야", "이 줄은 지연이야"라는 정답지(Label)가 필요합니다. 
아까 제가 제안드린 대로 save_data.py를 수정하거나, 수집이 끝난 후 엑셀/파이썬에서 Status 값을 숫자로 변환해야 합니다.정상 = 0, 지연 = 1, 손실 = 2, 혼합 = 3

3단계: 구글 코랩(Colab) 분석 및 모델링   
우분투에서 수집한 data.csv를 내보내어 구글 코랩에서 다음 과정을 수행합니다. (데이터 수집이 끝나면 제가 코드를 짜드릴게요.)
시각화: RTT 분포 그래프, 손실률 차트 그리기.데이터 분할: 회의 내용대로 Train(80%) / Test(20%)로 나눕니다.   
모델 선정: Isolation Forest나 Random Forest 같은 알고리즘으로 학습을 돌립니다.평가: 정확도(Accuracy)와 정밀도(Precision) 수치를 뽑습니다.

4단계: 논문 근거 기반 Threshold(임계치) 재설정   
analysis.py에서 했던 95% Percentile 방식도 좋지만, "논문 기재 바탕"이라는 조건을 위해 3-Sigma($\mu \pm 3\sigma$) 공식을 적용한 결과를 보고서에 추가합니다.   
정상 데이터의 평균($\mu$)과 표준편차($\sigma$)를 계산하여 임계치를 도출하는 과정을 코드로 보여주면 신뢰성이 확 올라갑니다.

5단계: 최종 Rule DB 및 대응 로직 구현 (rule.py)   
AI 모델이 "이상"이라고 판단했을 때, 사용자에게 보여줄 메시지를 매핑합니다.   
Delay 감지 시: "네트워크 대역폭 확인 및 스위치 점검 요망"   
Loss 감지 시: "물리적 케이블 손상 및 전자기 간섭 확인 요망"
